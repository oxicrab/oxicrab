...
- **Landlock sandbox** (`src/utils/sandbox/mod.rs`): Linux kernel-level Landlock LSM sandboxing applied to shell commands via `pre_exec`. Default read-only: `/usr`, `/lib`, `/lib64`, `/bin`, `/sbin`, `/etc`. Default read-write: workspace dir, `/tmp`, `/var/tmp`. Network blocked by default. Config under `tools.exec.sandbox`: `enabled` (default true), `additionalReadPaths`, `additionalWritePaths`, `blockNetwork` (default true). Degrades gracefully (BestEffort) on older kernels; no-op on non-Linux.
