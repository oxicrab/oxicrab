...
- **Auto-continue on mid-task stops**: When the LLM returns text after making 3+ tool calls in the current run, the loop re-prompts ("You stopped mid-task...continue with the remaining work") up to 2 times before accepting the text response. Prevents premature exits on complex multi-step tasks. Tracked via `total_tool_calls` and `auto_continue_count` counters in `run_agent_loop_with_overrides()`.
