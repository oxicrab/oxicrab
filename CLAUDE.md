...
- **Gateway HTTP API**: `src/gateway/mod.rs` provides an axum-based REST server with `POST /api/chat` and `GET /api/health`. `HttpApiState` holds `inbound_tx` (to publish to the agent) and `pending: Arc<Mutex<HashMap<String, oneshot::Sender<OutboundMessage>>>>` for synchronous request-response over the async `MessageBus`. `chat_handler` creates a oneshot channel, stores the sender in the pending map keyed by request ID (`http-{uuid}`), publishes an `InboundMessage` with `channel="http"`, and awaits the receiver with a 120s timeout. `route_response()` intercepts outbound messages where `channel=="http"`, routes them to the matching pending oneshot, and returns `true` (consumed). Called in `start_channels_loop` before channel dispatch. `start()` returns `(JoinHandle, HttpApiState)`. Axum is a non-optional dependency (used by both gateway and Twilio webhook).
