...
- **Adding fields to `AgentLoopConfig`**: Tool-specific configs go in `ToolConfigs` (forwarded to `ToolBuildContext`); non-tool fields go in `AgentLoopConfig` directly. Must update `from_config()`, `test_defaults()`, destructure in `AgentLoop::new()`, AND update `tests/common/mod.rs` `create_test_agent_with()`.
...
- **No `tool_choice` forcing**: The agent loop uses `tool_choice=None` (auto) for all iterations. Do not re-add `tool_choice="any"` — it breaks conversational flow and disables hallucination detection by setting `any_tools_called=true`. Hallucination safety comes from `handle_text_response()` in `src/agent/loop/mod.rs`.
...
- **CLI `stats` command**: `oxicrab stats today|costs|search|intent` queries the memory database for cost and search metrics.
...
- **Model routing**: `ModelRoutingConfig` in `src/config/schema/agent.rs` with `tiers`, `rules`, `fallbacks`. `ResolvedRouting` in `src/config/routing.rs` resolves task types to `(Arc<dyn LLMProvider>, String)`. `AgentRunOverrides.provider` enables cross-provider routing. `AgentLoop::resolve_overrides(task_type)` is the entry point. When `modelRouting` is absent, behavior is unchanged.
- **Complexity-aware message routing**: `ComplexityScorer` in `src/agent/loop/complexity/mod.rs`. Config: `modelRouting.complexity` with `enabled` (default false), `thresholds` (`lightStandard`/`standardHeavy`), `tierMapping` (light/medium/heavy → tier names), `weights` (7 dimensions). Scores each inbound message using AC automata + regex (sub-millisecond, zero API calls). Dimensions: message length (sigmoid), reasoning keywords (AC, saturates at 3), technical vocabulary (AC, saturates at 5), question complexity (regex tiers), code presence, instruction complexity, conversational simplicity (negative weight). Force overrides: 2+ reasoning keywords → heavy, pure greeting/filler → lightweight, >50KB → heavy. Composite via `sigmoid(weighted_sum - 0.35, 6.0)`. Tier resolution via `ResolvedRouting::resolve_tier_direct()`. Wired in `process_message_unlocked()` after intent classification.
...
- **Knowledge directory (RAG ingestion)**: `workspace/knowledge/` is indexed alongside `workspace/memory/` by `MemoryIndexer`. Supports `.md`, `.txt`, and `.html` files (HTML has tags stripped via `scraper`). Recurses into subdirectories via `walkdir` — source keys use relative paths (e.g. `knowledge:subdir/faq.md`). Knowledge entries appear in hybrid search results, are NOT subject to archive/purge (hygiene skips `knowledge:` prefixed entries), and ARE included in group chats (shared reference, not personal). Orphaned knowledge entries (files deleted from disk) are automatically cleaned up during re-indexing. `MemoryStore::knowledge_dir()` exposes the path. `MemoryDB::index_knowledge_directory()` handles indexing. Directory is optional — created by the user, indexed if present.
