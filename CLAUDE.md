...
Requires **Rust nightly** (pinned to `nightly-2026-03-05` in CI) and system deps: `cmake`. Voice transcription also requires `ffmpeg`. TLS uses rustls (pure Rust, no OpenSSL dependency).
...
- Implement `Tool` trait: `name()`, `description()`, `parameters()`, `capabilities()`, `execute(params, ctx)`
...
- **Adding a new tool**: Add a `register_*()` function in `src/agent/tools/setup.rs`, call it from `register_all_tools()`. Update `README.md` and the workspace file `AGENTS.md` if it exists.
