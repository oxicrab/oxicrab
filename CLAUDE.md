...
- **Group access control**: Channels support `allowGroups` in the TOML schema. Empty list = all groups allowed (backward compatible). Non-empty list restricts to listed group/channel IDs. Shared `check_group_access()` in `channels/utils`. Currently supported on Telegram, Discord, and Slack.
