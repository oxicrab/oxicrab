...
- **Skill hygiene**: Startup hygiene calls `prune_unused_skill_index(now_ms, 30_days_ms, min_uses=1)` from `crates/oxicrab-memory/src/hygiene/mod.rs::run_hygiene`. Boundary is inclusive (`<=`) so a skill exactly 30 days old is dropped — matches "older than 30 days" semantics.
### CLI & Config
