...
- **Startup model verification**: At gateway startup, all configured models (primary, fallbacks, routing overrides) are verified with a 1-token probe. Controlled by `agents.defaults.startupCheck`: `"warn"` (default, log and continue), `"fatal"` (abort if any model unreachable), `"off"` (skip). Verification uses `verify_all_providers()` in `src/providers/verify.rs`. Runs in parallel with 30s per-model timeout. Routing providers are created once and shared between verification and runtime (not double-constructed).
