...
- **Memory citations**: `memory_search` results include both a `**{source_key}**:` heading prefix AND an `_Source: {source_key}_` trailer in direct chats; group chats keep just the prefix to avoid context bloat. The trailer signals to the model that this is a quotable source, raising grounding. Adopted from openclaw's `memory.citations: auto` mode. Mechanically implemented in `MemoryStore::get_memory_context_scoped()` with `is_group` controlling the format.
- **Recall-driven memory promotion** (`agents.defaults.memory.promotion`): off by default. When enabled, the daily maintenance ticker queries `memory_access_log` + `memory_search_hits` for `daily:` entries that crossed `minRecalls` (default 5) AND `minUniqueQueries` (default 2) within `daysBack` (default 30), then rewrites their `source_key` from `daily:…` to `knowledge:auto:…` so they survive retention purge (knowledge entries are exempt). DB methods `find_promotion_candidates` + `promote_source_key` in `crates/oxicrab-memory/src/memory_db/stats.rs`. Adopted from openclaw's `recordShortTermRecalls` + `short-term-promotion` pattern.
- **Structured claims** (migration #11, `crates/oxicrab-memory/src/memory_db/claims.rs`): two tables — `claims` (id, text, confidence 0..1, status, last_seen_ms, timestamps) and `claim_evidence` (typed pointer kind + value). Status lifecycle: `open → accepted | retracted | contradicted`. `claim` tool (`src/agent/tools/claim/`) exposes five actions to the LLM: `add` / `list` / `get` / `lint` / `update_status`. The `lint` action runs three passes (contradiction via token-overlap + opposite-polarity heuristic, stale-low-confidence ≤ 0.4 conf + > 90d, orphan = no evidence) and returns a markdown report. The `get` action bumps `last_seen_ms` (re-observation signal). Pairs with the `_Source:` retrieval citation: citations cover retrieval grounding, claims cover ingestion grounding (low-confidence hedges no longer round to fact). Adopted from openclaw's claims/wiki_lint pattern. **Re-entrant lock care**: `list_claims` / `find_*` collect ids while holding `std::sync::Mutex<Connection>`, then drop the lock before iterating `get_claim` per id — the mutex is not re-entrant.
- **Tool-replay-repair after compaction** (`src/agent/compaction/mod.rs`): when compaction drops a `tool_result` whose matching `tool_use` survived in an earlier message, the orphaned `tool_use` no longer gets stripped — instead a synthesized placeholder tool message ("(tool result was dropped by compaction; treat as unknown outcome…)") is inserted right after the orphaning assistant turn. Preserves the "I tried X" structure of the conversation and satisfies provider strict-pairing requirements (Anthropic). API: `strip_orphaned_tool_messages` defaults to repair=true; legacy strip-only via `repair_orphaned_tool_messages(msgs, false)`. Adopted from openclaw's tool-replay-repair pattern.
- **Phantom tool_calls guard** (`LLMResponse::is_tool_use_finish`): some API gateways inject a `tool_calls` block while `finish_reason` indicates a real stop (`content_filter`, `error`, `stop`). The agent loop now clears phantom blocks and warns. Adopted from nanobot PR #3225.
- **LLM request timeout** (`agents.defaults.llmRequestTimeoutSeconds`, default 300, 0 disables): wraps the model_gateway invoke in `tokio::time::timeout` so a hung provider doesn't hold the per-session lock indefinitely. Adopted from nanobot PR #3428.
- **Pre-compaction watermark on hard failure**: when the LLM extraction call hard-fails (network error, provider down), the `pre_flush_msg_count` watermark advances anyway — preventing a runaway cost loop where every compaction re-tries the same chunk. Adopted from nanobot PR #3413/14.
- **autoSuggest LLM body** (`agents.defaults.trajectory.autoSuggest.useLlmBody`, default false): when true, the trajectory-driven skill auto-suggest pipeline fires a small LLM call to write a purpose-specific skill markdown for staged candidates instead of the fixed template. Falls back to template on LLM failure. Adopted from OpenCrust's auto-skill writer.
...
- **Skill propose / promote (`src/agent/skills/propose.rs`)**: `propose_skill` writes a candidate to `<workspace>/skills/staged/<name>.md`; staged skills are NOT loaded into context. `promote_staged_skill` is hardened against TOCTOU: uses `symlink_metadata` to refuse symlinks, re-runs `scanner::scan_skill` on the freshly-read content, then `std::fs::rename`. `MAX_PROPOSED_SKILL_BYTES = 32 KB` on propose, `MAX_STAGED_READ_BYTES = 64 KB` on promote. Skill names must match `[A-Za-z0-9][A-Za-z0-9_-]{0,63}` (no leading `-`/`_`, no path components). The Tool wrapper that exposes these helpers to the LLM is not yet registered — operators call them via API/cron until that lands.
...
- **LLM-as-Judge (`agents.defaults.judge`)**: Poison-resistant semantic gate for tool calls in `src/agent/loop/judge.rs`. Fires inside `execute_tool_call` (helpers.rs) AFTER the operator approval workflow and BEFORE param validation + `registry.execute`. The judge LLM sees only `(tool_name, scrubbed_args, user_intent)` — never conversation history or prior tool results — so an injected web page can't poison the gate with the same payload that poisoned the agent. Returns `{verdict: allow|block, reason: ≤200 chars}`. Block surfaces the reason as a tool error so the agent can re-plan. Fail-open on timeout/provider-error/malformed-JSON. Bounded by `maxTokens` (default 200) + `timeoutSeconds` (default 5). Config supports `allowedTools` / `blockedTools` for per-tool rollout (allow wins). Off by default. Adopted from IronClaw PR #2845.
- **Credential scrubber (structural, complementary to `LeakDetector`)**: `oxicrab_safety::scrub_credentials_in_json` / `scrub_credentials_in_text` (`crates/oxicrab-safety/src/credential_scrubber/`) replace the value of any structured `Authorization` / `api_key` / `token` / `secret` / `password` / `access_token` field (case-insensitive) with `[REDACTED]`. Walks JSON recursively and parses URL query strings (`?api_key=…&token=…`). Catches arbitrary `Bearer xxx` / `?api_key=USER_VAR` shapes that `LeakDetector` (which only knows known secret *patterns*) misses. Applied at four recording boundaries — `summarize_params` and `summarize_result` in cron traces (`memory_db/traces/mod.rs`), `insert_dlq_entry` (`memory_db/dlq.rs`), and `insert_subagent_log` (`memory_db/subagent_log/mod.rs`) — so future callers automatically benefit. Always-on, no config knob. Adopted from IronClaw PR #2529.
