...
- **Trusted proxy rate limiting** (`crates/oxicrab-gateway/src/`): `gateway.rateLimit.trustProxy` only trusts `X-Forwarded-For` when the socket peer matches one of `gateway.rateLimit.trustedProxies` (exact IP or CIDR). This prevents direct clients from spoofing rate-limit identity by sending their own forwarded headers.
