...
mod response_format;
pub mod status;
pub use response_format::response_format_from_json;
...
use oxicrab_core::bus::InboundMessage;
use oxicrab_core::bus::OutboundMessage;
use oxicrab_core::config::schema::{WebhookConfig, WebhookTarget};
use oxicrab_core::safety::LeakRedactor;

type HmacSha256 = Hmac<Sha256>;

/// Crate version, used in health/status endpoints.
...
///
/// Converted to the provider-level [`ResponseFormat`](oxicrab_core::providers::base::ResponseFormat)
...
impl GatewayResponseFormat {
    ...
    fn into_response_format(self) -> oxicrab_core::providers::base::ResponseFormat {
        match self {
...
async fn chat_handler(
    ...
    let mut builder = InboundMessage::builder("http", "http-api", request_id.clone(), body.message)
        .meta(
            oxicrab_core::bus::meta::SESSION_ID,
            serde_json::Value::String(session_id.clone()),
        );
    if let Some(ref rf) = response_format_value {
        builder = builder.meta(
            oxicrab_core::bus::meta::RESPONSE_FORMAT,
...
async fn webhook_handler(
    ...
    if let Some(ref dispatch_config) = config.dispatch {
        ...
        let dispatch = oxicrab_core::dispatch::ActionDispatch {
            ...
            params,
            source: oxicrab_core::dispatch::ActionSource::Webhook {
...
/// A no-op leak redactor for testing.
pub struct NoopRedactor;

impl LeakRedactor for NoopRedactor {
