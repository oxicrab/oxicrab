...
impl AnthropicOAuthProvider {
    ...
    fn load_cached_tokens_from_file(&self, path: &Path) {
        let data = match crate::utils::read_json_locked::<Value>(path) {
            Ok(Some(v)) => v,
            Ok(None) => return,
            Err(e) => {
    ...
    async fn save_credentials(&self) {
        ...
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
            crate::utils::write_json_locked(&path, &data)?;
            Ok(())
        })
        .await;
    ...
    pub fn from_credentials_file(
        ...
        let Some(data) = crate::utils::read_json_locked::<Value>(path)? else {
            return Ok(None);
        };
