...
if [ ! -f /etc/oxicrab/config.toml ]; then
    if [ -f /usr/share/doc/oxicrab/config.example.toml ]; then
        install -m 0640 -o oxicrab -g oxicrab \
            /usr/share/doc/oxicrab/config.example.toml \
            /etc/oxicrab/config.toml
        echo "oxicrab: installed example config to /etc/oxicrab/config.toml"
